"""Part segmentation via fit-and-remove: fit a dominant primitive, cluster
the leftover high-residual points, fit a second primitive to any large
enough cluster."""
import numpy as np
...
def segment_and_fit(raw_cloud, residual_threshold=0.35, cluster_radius=0.01,
                     min_cluster_size=40, verbose=True, max_nfev=3000):
    dominant_params, dominant_info = fit_superquadric(raw_cloud, max_nfev=max_nfev)
    ...
    if verbose:
        print(f'Dominant fit: rmse={dominant_info["rmse"]:.5f}, '
              f'{explained_mask.sum()}/{len(raw_cloud)} points explained, {len(leftover_idx)} leftover')
    ...
    for i, cluster_local_idx in enumerate(clusters):
        ...
        if len(cluster_cloud) < 8:
            continue
        part_params, part_info = fit_superquadric(cluster_cloud, max_nfev=max_nfev)
