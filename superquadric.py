...
def fit_superquadric(points, init=None, verbose=0, max_size_multiplier=None,
                      min_size_multiplier=None, position_margin_multiplier=None,
                      max_nfev=3000):
    """Fit a superquadric to a point cloud. Optional size/position bounds
    (relative to the input cloud's own bounding box) prevent the optimizer
    from finding degenerate solutions on small/sparse clusters -- found
    necessary when fitting leftover clusters during iterative segmentation."""
    if init is None:
    ...
    result = least_squares(resid_fn, x0, bounds=(lo, hi), method='trf',
                            loss='soft_l1', f_scale=0.05, max_nfev=max_nfev, verbose=verbose)

    fitted = vec_to_params(result.x)
    final_resid = resid_fn(result.x)
    info = {'success': result.success, 'cost': result.cost,
            'rmse': float(np.sqrt(np.mean(final_resid ** 2))),
            'max_abs_resid': float(np.max(np.abs(final_resid))), 'nfev': result.nfev}
    return fitted, info
