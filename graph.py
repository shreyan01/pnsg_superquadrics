"""General adjacency-graph representation for multi-part objects."""
...
class PartNode:
    ...
    def principal_axis(self) -> np.ndarray:
        ...
        tip = local_to_world(np.array([[0, 0, 1.0]]), 0, 0, 0,
                              self.params['roll'], self.params['pitch'], self.params['yaw'])[0]
        return tip / (np.linalg.norm(tip) + 1e-9)

    def size_along_axis(self) -> float:
        return self.params['a3']

    def horizontal_extent(self) -> float:
        return max(self.params['a1'], self.params['a2'])


def classify_relation(a: PartNode, b: PartNode) -> dict:
    offset = b.centroid - a.centroid
    dist = float(np.linalg.norm(offset))
    horiz = float(np.linalg.norm(offset[:2]))
    vert = float(offset[2])
    axis_dot = float(np.dot(a.principal_axis, b.principal_axis))
    coaxial = abs(axis_dot) > 0.9

    if horiz > 0.4 * a.horizontal_extent() and abs(vert) < a.size_along_axis():
        attach = 'side'
    elif vert > 0.6 * a.size_along_axis():
        attach = 'top'
    elif vert < -0.6 * a.size_along_axis():
        attach = 'bottom'
    else:
        attach = 'overlapping_or_unclear'
...
class ObjectGraph:
    ...
    def add_node(self, node: PartNode):
    ...
    def build_edges_mst(self):
        ...
        if len(self.nodes) < 2:
            return
        in_tree = {self.nodes[0].node_id}
        remaining = {n.node_id: n for n in self.nodes[1:]}
        ...
        while remaining:
            ...
            self.edges.append((a_id, b_id, relation))
            in_tree.add(b_id)
            del remaining[b_id]
    ...
    def describe(self) -> str:
        ...
        for n in self.nodes:
            ...
            lines.append(f'  node {n.node_id} [{role}]: size=({n.params["a1"]*1000:.0f},'
                          f'{n.params["a2"]*1000:.0f},{n.params["a3"]*1000:.0f})mm '
                          f'eps=({n.params["eps1"]:.2f},{n.params["eps2"]:.2f})')
...
def fit_graph_from_segmented_clouds(labeled_clouds: dict) -> ObjectGraph:
    graph = ObjectGraph()
